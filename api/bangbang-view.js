...
export default async function handler(request, response) {
    ...
    try {
        // 获取仓库根目录的所有文件和目录
        const repoContents = await axios.get(`https://api.github.com/repos/${owner}/${repo}/contents`);
        ...
        if (imageFiles.length === 0) {
            ...
            response.end(JSON.stringify({ error: "No image files found in the repo's root directory." }));
            return;
        }
